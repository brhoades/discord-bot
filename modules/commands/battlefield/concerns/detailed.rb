...
module BF1
  module DetailedStats
    ...
    def pretty_kit_statistics(name)
      ...
      maxNameSize = kits.max { |a, b| a.dig("name")&.length <=> b.dig("name")&.length }
                      .dig("name")&.length
      maxKills = kits.max { |a, b| a.dig("kills")&.round(0)&.to_s&.length <=> b.dig("kills")&.round(0)&.to_s&.length }
                   .dig("kills")&.round(0)&.to_s&.length
